...
def setup_logging(log_root, log_header, dataset_name, model_ckpt, note=None):
    model_name = model_ckpt.split("/")[-1]
    if note:
        log_dir = os.path.join(log_root, log_header, dataset_name, model_name, f'{datetime.now().strftime("%Y-%m%d-%H%M")}' + " [" + note + "]")
    else:
        log_dir = os.path.join(log_root, log_header, dataset_name, model_name, f'{datetime.now().strftime("%Y-%m%d-%H%M")}')
...
def data_reader(args):
    ...
    elif args.dataset_name == "multiarith":
        dataset_path = os.path.join(args.data_root, args.dataset_name, "test.json")
    elif args.dataset_name == "svamp":
        dataset_path = os.path.join(args.data_root, args.dataset_name, "test_with_ids.json")
    ...
    elif args.dataset_name in ("addsub", "multiarith", "svamp"):
      with open(dataset_path) as f:
        json_data = json.load(f)
        for line in json_data:
          q = line["question"].strip()
          try:
            a = str(line["final_ans"])
          except:
            a = str(line["answer"])
          questions.append(q)
          answers.append(a)
...
def answer_cleansing(args, pred):
    ...
    if args.dataset_name in ("aqua", "commonsensqa"):
        ...
        pred = re.sub("\"|\'|\n|\.|\s","", pred)
        pred = [pred]
    elif args.dataset_name == 'mmlumath':
        pred = re.findall(r'A|B|C|D|E|F', pred)
    else:
        raise ValueError("dataset is not properly defined ...")
